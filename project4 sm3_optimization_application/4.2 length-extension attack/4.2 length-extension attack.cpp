...
vector<u8> sm3_padding(size_t msg_len_bytes) {
    ...
    //填充0x00，直到消息长度（含padding）对64取余为56（即512位 - 64位长度）
...
int main() {
    ...
    //构造伪造消息 = original_msg + padding(original_msg) + append_msg
    ...
    //计算长度扩展攻击哈希，即从Hash(M)开始，继续压缩append_msg部分
    ...
    cout << "从扩展状态计算的哈希值(M || padding(M) || append_m): ";
    ...
    if (extended_hash == direct_hash) {
        cout << "长度扩展攻击验证成功：哈希值匹配。" << endl;
    }
    else {
        cout << "长度扩展攻击失败：哈希值不匹配。" << endl;
    }
