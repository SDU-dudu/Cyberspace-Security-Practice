...
// GCM模式加密主体函数
void GCM(const uint32_t rk[32], const vector<uint8_t>& IV,
    const vector<uint8_t>& plaintext, vector<uint8_t>& ciphertext,
    uint8_t res[16]) {
    ...
    if (IV.size() == 12) {
        ...
        u128 res_ghash = GHASH(H, tmp);
        for (int i = 0; i < 8; i++) count[i] = (res_ghash.high >> (56 - 8 * i)) & 0xFF;
        for (int i = 0; i < 8; i++) count[8 + i] = (res_ghash.low >> (56 - 8 * i)) & 0xFF;
    }
    ...
    if (ciphertext.size() % 16) tmp.insert(tmp.end(), 16 - (ciphertext.size() % 16), 0);
    for (int i = 0; i < 8; i++) tmp.push_back(0);
    uint64_t c_bits = (uint64_t)ciphertext.size() * 8;
    for (int i = 0; i < 8; i++) tmp.push_back((c_bits >> (56 - 8 * i)) & 0xFF);
...
int main() {
    ...
    cout << fixed << setprecision(7)<< "GCM模式加密用时:" << time << "秒" << endl;
    cout << "加密结果为：";
    for (auto b : res) {
