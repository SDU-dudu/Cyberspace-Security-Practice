...
static __m128i SM4_SBox(__m128i x) {
    ...
    x = _mm_shuffle_epi8(x, MASK);  // 字节重排列

    // SM4 S盒的复合域实现
    x = AddTC(MulMatrixTA(x));
    x = _mm_aesenclast_si128(x, _mm_setzero_si128()); // AES指令加速
    return AddATAC(MulMatrixATA(x));
}

// ============================= SM4 加解密核心函数 (AES-NI加速) =============================
...
// 公开的加密函数接口 (一次处理8个块)
void SM4_AESNI_Encrypt_x8(uint8_t* plaintext, uint8_t* ciphertext, SM4_Key* sm4_key) {
    SM4_AESNI_do(plaintext, ciphertext, sm4_key, 0); // 0表示加密
}

// 公开的解密函数接口 (一次处理8个块)
void SM4_AESNI_Decrypt_x8(uint8_t* ciphertext, uint8_t* plaintext, SM4_Key* sm4_key) {
    SM4_AESNI_do(ciphertext, plaintext, sm4_key, 1); // 1表示解密
}
...
public:
    ...
    void UpDate() {
        _begin = std::chrono::steady_clock::now(); // 更新时间起点
    }

    double GetSecond() {
        _end = std::chrono::steady_clock::now(); // 获取当前时间
        std::chrono::duration<double> temp =
            std::chrono::duration_cast<std::chrono::duration<double>>(_end - _begin);
        return temp.count(); // 返回经过的秒数
    }
};
