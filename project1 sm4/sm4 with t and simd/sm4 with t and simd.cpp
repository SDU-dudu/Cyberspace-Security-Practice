...
void _SM4_do(uint8_t* in, uint8_t* out, SM4_Key sm4_key, int enc) {
    ...
    // 加载输入数据
    Temp[0] = _mm256_loadu_si256((__m256i*)in + 0);
    Temp[1] = _mm256_loadu_si256((__m256i*)in + 1);
    Temp[2] = _mm256_loadu_si256((__m256i*)in + 2);
    Temp[3] = _mm256_loadu_si256((__m256i*)in + 3);
    ...
    // 重新排列字节以正确对齐
    __m256i vindex = _mm256_setr_epi8(
...
int SM4_KeyInit(uint8_t* key, SM4_Key* sm4_key) {
    *sm4_key = (SM4_Key)malloc(32 * sizeof(uint32_t));
    if (*sm4_key == NULL) {
        return 1; // 内存分配失败
    }
    ...
    for (int i = 8; i < 32; i++) {
        uint32_t tmp = (*sm4_key)[i - 8];
        tmp = (tmp << 11) | (tmp >> (32 - 11));
        tmp ^= FK[(i - 8) % 4];
        tmp ^= CK[i - 8];
        (*sm4_key)[i] = tmp;
    }
...
int main() {
    ...
    cout << endl << "SM4 加密（使用 SIMD 和表）的时间： " << fixed << setprecision(7) << time1 << " 秒" << endl << endl;
    cout << endl << "SM4 解密（使用 SIMD 和表）的时间： " << fixed << setprecision(7) << time2 << " 秒" << endl << endl;
